...
public interface ProgramaRepository extends JpaRepository<Programa, Integer> {
	...
	@Query("select x from tb_programa x where (?1 is '' or x.id_programa like ?1) and (?2 is '' or x.id_categoria like ?2) and (?3 is '' or x.nom_prog = ?3) and (?4 is -1 or x.precio = ?4)")         
